...
#include <yumi_hw/YumiGrasp.h>
 ...
 void yumi_dialbox::JointStatesCallback(const sensor_msgs::JointState::ConstPtr& msg){
	...
	if (START){
		joint_states.points[0].positions[0]=msg->position[0]; //1_l
		joint_states.points[0].positions[1]=msg->position[2]; //2_l
		joint_states.points[0].positions[2]=msg->position[4]; //3_l
		joint_states.points[0].positions[3]=msg->position[6]; //4_l
		joint_states.points[0].positions[4]=msg->position[8]; //5_l
		joint_states.points[0].positions[5]=msg->position[10]; //6_l
		joint_states.points[0].positions[6]=msg->position[12]; //7_l
		joint_states.points[0].positions[7]=msg->position[1]; //1_r
		joint_states.points[0].positions[8]=msg->position[3]; //2_r
		joint_states.points[0].positions[9]=msg->position[5]; //3_r
		joint_states.points[0].positions[10]=msg->position[7]; //4_r
		joint_states.points[0].positions[11]=msg->position[9]; //5_r
		joint_states.points[0].positions[12]=msg->position[11]; //6_r
		joint_states.points[0].positions[13]=msg->position[13]; //7_r
		START=false;
...
int main(int argc, char **argv)
{
  ...
  joint_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/yumi/joint_trajectory_pos_controller/command", 10);
